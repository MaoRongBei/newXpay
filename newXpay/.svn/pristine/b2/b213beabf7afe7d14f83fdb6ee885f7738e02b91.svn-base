...
public class JdbcDao implements InitializingBean{
    ...
    public JdbcDao() {
		...
		File file = new File(System.getProperty("user.dir")+File.separator+"conf"+File.separator+"druid.properties");
		InputStream input = null;
		try {
			input = new FileInputStream(file);
			properties.load(input);
		} catch (IOException e) {
			logger.error("载入Properties出现异常", e);
		} finally {
			try {
				if (input != null) {
					input.close();
		...
		try {
			...
			logger.error("连接池初始化出错……", e);
		}
		logger.info("init连接池初始化成功");
    }
