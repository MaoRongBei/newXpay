...
import com.hrtpayment.xpay.cups.sdk.BaseHttpSSLSocketFactory.TrustAnyHostnameVerifier;
/**
 * 
...
public class HttpClient {
	...
	public boolean isIfValidateRemoteCert = false;
...
//			LogUtil.writeLog("请求报文:[" + sendData + "]");
	...
	public int sendJSON(String jsonStr, String encoding) throws Exception {
		try {
			HttpURLConnection httpURLConnection = createConnectionJSON(encoding);
			if (null == httpURLConnection) {
				throw new Exception("Create httpURLConnection Failure");
			}
			LogUtil.writeLog("请求报文:[" + jsonStr + "]");
			this.requestServer(httpURLConnection, jsonStr,
					encoding);
			this.result = this.response(httpURLConnection, encoding);
			LogUtil.writeLog("Response message:[" + result + "]");
			return httpURLConnection.getResponseCode();
	 ...
	 * @throws Exception
	 */
	public int sendMultipart(Map<String, String> data,
			Map<String, FileItem> fileParams, String encoding) throws Exception {
		OutputStream out = null;
		StringBuffer reqSb = new StringBuffer("");
		try {
			String boundary = System.currentTimeMillis() + ""; // 随机分隔线
			HttpURLConnection httpURLConnection = createConnectionMultipart(encoding,boundary);
			if (null == httpURLConnection) {
				throw new Exception("Create httpURLConnection Failure");
			}
			...
			byte[] entryBoundaryBytes = ("\r\n--" + boundary + "\r\n").getBytes(encoding);
			 // 组装文本请求参数
            Set<Entry<String, String>> textEntrySet = data.entrySet();
            for (Entry<String, String> textEntry : textEntrySet) {
                byte[] textBytes = getTextEntry(textEntry.getKey(), textEntry.getValue(),encoding);
                out.write(entryBoundaryBytes);
                out.write(textBytes);
                reqSb.append(new String(entryBoundaryBytes)).append(new String(textBytes));
            }
            ...
            for (Entry<String, FileItem> fileEntry : fileEntrySet) {
                ...
                byte[] fileBytes = getFileEntry(fileEntry.getKey(), fileItem.getFileName(),
                    fileItem.getMimeType(), encoding);
                out.write(entryBoundaryBytes);
                out.write(fileBytes);
                out.write(fileItem.getContent());
                reqSb.append(new String(entryBoundaryBytes)).append(new String(fileBytes)).append("二进制内容这里不显示了");
            }
            // 添加请求结束标志
            byte[] endBoundaryBytes = ("\r\n--" + boundary + "--\r\n").getBytes(encoding);
            out.write(endBoundaryBytes);
            reqSb.append(new String(entryBoundaryBytes));
            this.result = getResponseAsString(httpURLConnection);
            LogUtil.writeLog("request message:[" + reqSb.toString() + "]");
			LogUtil.writeLog("Response message:[" + result + "]");
			return httpURLConnection.getResponseCode();
		} catch (Exception e) {
			throw e;
	 ...
	 */
	private HttpURLConnection createConnectionMultipart(String encoding,String boundary) throws ProtocolException {
		...
		httpURLConnection.setUseCaches(false);// 取消缓存
		httpURLConnection.setRequestProperty("Content-type",
				"multipart/form-data;boundary=" + boundary + ";charset=" + encoding);
		httpURLConnection.setRequestMethod("POST");
		if ("https".equalsIgnoreCase(url.getProtocol())) {
			HttpsURLConnection husn = (HttpsURLConnection) httpURLConnection;
			//是否验证https证书，测试环境请设置false，生产环境建议优先尝试true，不行再false
	  ...
	  private static byte[] getFileEntry(String fieldName, String fileName, String mimeType,
	    ...
	    private static String getResponseCharset(String ctype) {
	        String charset = ApiConstants.DEFAULT_CHARSET;
