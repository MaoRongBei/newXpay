...
@Service
public class CupsATMerchantService {
	...
	@Value("${cupsWx.queryMerUrl}")
	String queryMerUrl;
	@Value("${cupsWx.addMerUrl}")
	String addMerUrl;
	@Value("${cupsAli.url}")
	String aliMerUrl;
	@Value("${cupsWx.md5Key}")
	...
	public String addAliMerchants(Map<String, Object> map) throws BusinessException {
		...
		String res = null;
		try {
			logger.info("[银联-支付宝]商户入驻请求信息----->" + req);
			res = HttpConnectService.postForm(req, reqUrl);
			logger.info("[银联-支付宝]商户入驻响应信息----->" + res);
		} catch (Exception e) {
			logger.info("[银联-支付宝]商户{}:{}入驻异常，错误 原因{}----->", map.get("hrid"), map.get("merchantid"), e.getMessage());
		...
		if ("10000".equals(rtnCode)) {
			...
			if (sub_merchant_id == null || "".equals(sub_merchant_id)) {
				...
				logger.error("[银联-支付宝]商户入驻失败，订单号{},子商户号未返回", resMap.get("out_trade_no"));
				throw new BusinessException(8000, "商户入驻失败");
			}
			...
			logger.error("[银联-支付宝]商户入驻失败-通信状态失败，hrid{}：商户号{},返回{}，{}", map.get("hrid"), map.get("merchantid"), rtnCode,
					rtnMsg);
			RedisUtil.addFailCountByRedis(1);
	...
	public String addWxMerchants(Map<String, Object> map) throws BusinessException {
		...
		merMsg.put("business", String.valueOf(map.get("category")));// 随机字符串
		merMsg.put("merchant_name", String.valueOf(map.get("merchantname")));// 条码
		merMsg.put("merchant_shortname", String.valueOf(map.get("shortname")));// 订单描述
		merMsg.put("service_phone", String.valueOf(map.get("servicephone")));// 订单金额
		merMsg.put("merchant_remark", String.valueOf(map.get("merchantid")));// 订单编号
//		merMsg.put("contact", String.valueOf(map.get("contactname")));
//		merMsg.put("contact_wechatid_type", "type_wechatid");
//		merMsg.put("contact_wechatid", String.valueOf(map.get("minfo2")));
		Map<String, String> req = cupsService.sign(merMsg);
		String reqStr=null;
		try {
		     reqStr=cupsService.mapToXml(req) ;
		} catch (Exception e1) {
			 logger.info("[银联-微信]商户入驻  请求报文转化异常：{}",e1.getMessage());
			 throw new BusinessException(8000, "商户入驻异常");
		}
		logger.info("请求信息----->" + req);
		String res = null;
		try {
			logger.info("[银联-微信]商户入驻请求信息----->" + reqStr);
			res = client.sendFormData(reqUrl, reqStr);//HttpConnectService.postForm(req, reqUrl);
			logger.info("[银联-微信]商户入驻响应信息----->" + res);
		} catch (Exception e) {
			logger.info("[银联-微信]商户{}:{}入驻异常，错误 原因{}----->", map.get("hrid"), map.get("merchantid"), e.getMessage());
			throw new BusinessException(8000, "商户入驻失败");
		}
		...
		try {
			respJson = cupsService.xmlToMap(res);
			 ...
			 logger.info("[银联-微信]商户入驻 响应报文转化异常：{}",e.getMessage());
		...
		if (!"SUCCESS".equals(respJson.get("return_code"))) {
			...
			RedisUtil.addFailCountByRedis(1); 
			logger.error("[银联 -微信] 商户入驻 失败{}：{}", respJson.get("return_code"), respJson.get("return_msg"));
			throw new BusinessException(1002, "商户入驻失败");
		} else {
			...
			if ("SUCCESS".equals(rtnCode)) {
				...
				if (sub_merchant_id == null || "".equals(sub_merchant_id)) {
					logger.error("[银联-微信]商户入驻失败，订单号{},子商户号未返回", respJson.get("mch_id"));
					throw new BusinessException(8000, "商户入驻失败");
				}
				...
				logger.error("[银联-微信]商户入驻失败-通信状态失败， 商户号{},返回{}，{}", map.get("merchantid"), rtnCode, rtnMsg);
				throw new BusinessException(1002, "商户入驻失败");
			}
		}
	}
	...
	public String queryAliMerchants(Map<String, Object> map) throws BusinessException {
		...
		String res = null;
		try {
			logger.info("[银联-支付宝]商户入驻查询请求信息----->" + req);
			res = HttpConnectService.postForm(req, reqUrl);
	...
	public String queryWxMerchants(Map<String, Object> map) throws BusinessException {
		...
		try {
			 ...
			 logger.info("[银联-微信]商户入驻查询  请求报文转化异常：{}",e1.getMessage());
			 throw new BusinessException(8000, "商户入驻查询异常");
		}
		logger.info("请求信息----->" + req);
		String res = null;
		try {
			logger.info("[银联-微信]商户入驻查询请求信息----->" + reqStr);
			res = client.sendFormData(reqUrl, reqStr);//HttpConnectService.postForm(req, reqUrl);
			logger.info("[银联-微信]商户入驻查询响应信息----->" + res);
		} catch (Exception e) {
			logger.info("[银联-微信]商户{}:{}入驻查询异常，错误 原因{}----->", map.get("hrid"), map.get("merchantid"), e.getMessage());
			throw new BusinessException(8000, "商户入驻查询失败");
		}
		Map<String, String> respJson=null;
		try {
		...
		if ("SUCCESS".equals(respJson.get("return_code"))) {
			if ("SUCCESS".equals(respJson.get("result_code"))) { 
				JSONArray mchinfoArray=JSONArray.parseArray(respJson.get("mchinfo"));
				JSONObject mchinfo=mchinfoArray.getJSONObject(0);//.get(0);
				String updateSql = "update Bank_MerRegister set merchantcode=?, approvestatus='Z' ,lmdate=sysdate,rtnmsg=?  "
						+ " where  (approvestatus<>'Y' or approvestatus is null )  and merchantid=? "; 
				int count=dao.update(updateSql, mchinfo.get("mch_id"),  getRtMsg(respJson.get("result_code") , respJson.get("result_msg")), mchinfo.get("merchant_remark"));
				logger.info("[银联-微信] 商户入驻查询：商户{}成功 {}",mchinfo.get("merchant_remark"),count);
				return "入驻成功" ;
			}else{
				String updateSql = "update Bank_MerRegister set rtnmsg=?  where approvestatus<>'Y' and  merchantid=? ";
				dao.update(updateSql, getRtMsg( respJson.get("result_code") ,  respJson.get("result_msg")), map.get("merchantid"));
				logger.error("[银联 -微信] 商户入驻查询  失败{}：{}", respJson.get("result_code"), respJson.get("result_msg"));
				throw new BusinessException(1002, "商户入驻查询失败");
			}
		}else{
			String updateSql = "update Bank_MerRegister set rtnmsg=?  where approvestatus<>'Y' and  merchantid=? ";
			dao.update(updateSql, getRtMsg( respJson.get("return_code") ,  respJson.get("return_msg")), map.get("merchantid"));
			logger.error("[银联 -微信] 商户入驻查询  失败{}：{}", respJson.get("return_code"), respJson.get("return_msg"));
			throw new BusinessException(1002, "商户入驻查询失败");
		}
		}
	...
	public String updateAliMerchants(String hrid) throws BusinessException {
		...
		String reqUrl =aliMerUrl;// "https://apay.test.95516.com/ali/aligateway";
		...
		String res = null;
		try {
			logger.info("[银联-AT]商户入驻修改请求信息----->" + req);
			res = HttpConnectService.postForm(req, reqUrl);
			logger.info("[银联-AT]商户入驻修改响应信息----->" + res);
		} catch (Exception e) {
			logger.info("[银联-AT]商户{}:{}入驻修改异常，错误 原因{}----->", map.get("hrid"), map.get("merchantid"), e.getMessage());
		...
		if ("10000".equals(rtnCode)) {
			String sub_merchant_id = resMap.get("sub_merchant_id");
			if (sub_merchant_id == null || "".equals(sub_merchant_id)) {
				logger.error("[银联-AT]商户入驻修改失败，订单号{},子商户号未返回", resMap.get("out_trade_no"));
				throw new BusinessException(8000, "商户入驻修改失败");
			}
			...
			logger.error("[银联-AT]商户入驻修改失败-通信状态失败，hrid{}：商户号{},返回{}，{}", map.get("hrid"), map.get("merchantid"), rtnCode,
					rtnMsg);
			RedisUtil.addFailCountByRedis(1);
	...
	public String addSubMerchant(String merchantCode,String zdlx,Map<String, Object> merchantInfo) throws BusinessException{
		...
		if ("1".equals(zdlx)) {
			...
			subMer.put("mch_id", String.valueOf(merchantInfo.get("mch_id")));// mchid);// 银行端提供 微信商户号
...
//			subMer.put("receipt_appid", String.valueOf(merchantInfo.get("appid")));
		...
		try {
			 ...
			 logger.info("[银联-微信]{}子商户配置【{}】  请求报文转化异常：{}",merchantCode,zdlx,e1.getMessage());
			 throw new BusinessException(8000, "商户入驻查询异常");
		}
		String res = null;
		try {
			logger.info("[银联-微信]子商户{}配置步骤【{}】请求信息----->{}" ,merchantCode,zdlx,reqStr);
			res = HttpConnectService2.sendMessage(reqStr, reqUrl,String.valueOf(merchantInfo.get("mch_id")));
			//,String.valueOf(merchantInfo.get("mch_id"))
			logger.info("[银联-微信]子商户{}配置步骤【{}】响应信息----->{}" ,merchantCode,zdlx, res);
		} catch (Exception e) {
			logger.info("[银联-微信]子商户{}配置步骤【{}】异常，错误 原因{}----->",merchantCode,zdlx, e.getMessage());
			...
			if("SUCCESS".equals(respJson.get("return_code"))){
				...
				if("1".equals(zdlx)){
					if("SUCCESS".equals(respJson.get("result_code"))){
						...
						updateStatus="Z";
						respMsg=respJson.get("err_code_des");
					}
				}else if("2".equals(zdlx)){
					if("SUCCESS".equals(respJson.get("result_code"))){
						updateStatus="Y";
			 ...
			 logger.info("[银联-微信]子商户{}配置步骤【{}】    响应报文转化异常：{}",merchantCode,zdlx,e.getMessage());
